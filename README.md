...
- A card links the repository, branch and pull request of the task's directory, the issues that pull request closes (needs `gh` 2.72.0 or later), and any Jira key (such as `PAY-418`) in the branch name or pull request title. Jira links need `task_dashboard.jira_url` in `config.yaml` (for example `https://example.atlassian.net`); a key links to `<jira_url>/browse/<key>`, and Jira itself is never queried. List your projects in `task_dashboard.jira_projects` (for example `[PAY, OPS]`) to link only their keys, so words such as `UTF-8` in a title are not taken for one.
