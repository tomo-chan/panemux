...
effect whenever it sees a status report addressed to `_agent-board`. `GET /api/board/status` reads only
...
`_agent-board` (the status-report recipient) and any pane on a *different* host (the cross-host relay
...
**Naming correction:** an earlier revision of this document (and PR #163's initial implementation)
used `_panemux` as this reserved identity. Renamed to `_agent-board` (`board.Sentinel` in
`internal/board`, `config.ReservedSentinelPaneID` in `internal/config`) because the sentinel is a
wire-level agmsg protocol identity, not an application name — coupling it to panemux's own current
product name meant a future rename of the application would also force a change to this protocol
constant, for no reason the protocol itself cares about. Every reference in this document, in code,
and in test fixtures uses `_agent-board` consistently; treat any remaining `_panemux` reference
anywhere in this codebase as stale.

Instead of a `kind='status'` field panemux owns (agmsg has no such column), status reports are
ordinary agmsg messages addressed to the reserved identity `_agent-board` — the same identity the
command center uses as its own `from` when sending. Because `_agent-board` is never an agmsg roster
...
forwarding, recognizes any row addressed to `_agent-board` as a status update and writes it into
...
positive edge: a human typing an ordinary chat message to `_agent-board` through the command center or
...
// relay later observes with From == "_agent-board" actually corresponds to one of panemux's own sends,
...
The relay inspects every `Row` it reads: if `To == "_agent-board"` and `Body` parses as JSON with
...
composed). A `Body` addressed to `_agent-board` that isn't valid JSON, or is valid JSON without that
...
[Testing plan](#testing-plan) bullet requiring "a row addressed to `_agent-board` updating `status` and
...
to `_agent-board`, and every ordinary cross-pane row that passes `from`/`to` validation (see
...
3. For each new row, panemux first checks `from`. If `from` is a board-enabled pane ID panemux
   knows about on that row's source host, the row passes. **If `from == "_agent-board"`, the row passes
   ...
   `_agent-board` as unconditionally trusted: because `send.sh --force` never checks `from` against a
   roster, any agent on any host can locally write a row claiming `from: "_agent-board"`, and nothing at
   ...
   with `from == "_agent-board"` that matches nothing in the ledger is a suspected forgery: it is dropped
   and logged the same as any other failed check, never relayed and never cached. Any other `from` —
   one that is neither a known local pane ID nor a ledger-matched `_agent-board` — is dropped and logged
   too. See [Security model](#security-model) for the forgery scenario this closes and why a
   universal `_agent-board` allowance (an earlier revision of this document's check) was not enough. If
   `from` passes: when `to == "_agent-board"`, panemux updates the
...
- **It reads and writes the board through panemux's own authenticated REST API — `GET
  ...
  intercepts `_agent-board`-addressed status reports before they ever leave the host they were written
...
- Sending a message is `POST /api/board/broadcast` with the command center's own reserved `from`
  identity, `_agent-board` — the exact same code path a human-triggered broadcast takes, which already
...
**`_agent-board` is reserved and validated where panemux can actually enforce it.** `internal/config/
validate.go` rejects any pane config whose `id` is literally `_agent-board`, the same way it already
...
nothing stops an operator or a live agent from running agmsg's own `join.sh <team> _agent-board ...`
...
`_agent-board` string by itself even after this validation — config-time reservation closes the
...
- **Universal `--force` makes `from` forgeable across the whole team, including impersonating
  `_agent-board` itself, unless the relay actively checks it.** `send.sh --force` accepts any `from`
  ...
  on Host A from sending a message with `from: "_agent-board"` to a pane on Host B, which
  [Cross-host relay](#cross-host-relay) would otherwise happily forward as if the command center had
  sent it. Unconditionally trusting any row with `from == "_agent-board"` would not close this — that
  ...
  `_agent-board`-attributed row is only accepted if it corresponds to a send panemux's own broadcast
...
- Agents/teams are free-text identifiers with no cryptographic authentication of `from`. The relay's
  ...
  match a known local pane ID or `_agent-board`, but within that set there is still no proof a given
...
- **A broadcast's delivery and its appearance in dashboard history are decoupled.**
  ...
  ledger](#package-layout) used for `_agent-board` forgery detection intentionally is not repurposed to
...
- `internal/board`: status JSON parsing (valid full payload with `kind: "board_status"`, missing
  ...
  *is* valid JSON, contains a `state` field, but is missing or has the wrong `kind` is also treated
  ...
  a row addressed to `_agent-board` updating `status` and *not* appearing in `history`'s cross-pane
  relay output (a plain non-status row addressed to `_agent-board` correctly *does* appear, as an
  ...
  known local pane ID on its source host nor a ledger-matched `_agent-board` is dropped and logged, never
  cached or relayed), the own-send ledger specifically (a `from == "_agent-board"` row that matches a
  recently recorded `Send` is accepted; a `from == "_agent-board"` row with no matching ledger entry —
  ...
  no longer matchable — this is the regression test for the cross-host `_agent-board` impersonation
  scenario in [Security model](#security-model), and for why an earlier revision's blanket
  `_agent-board` allowance was insufficient), empty team.
...
- `internal/config`: `host != loopback && auth_token == ""` is a validation error; all other
  ...
  `id: "_agent-board"` is a validation error, both alone and alongside otherwise-valid other panes.
