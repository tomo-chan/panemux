...
[agent-board.md's Architecture section](agent-board/architecture.md#architecture) describes), `ownSendLedger`
...
[security.md](security/agent-board.md#agent-board-remote-writes) describes). Three new optional session capability
...
(`internal/server/auth.go`) — see [security.md](security/auth.md#auth-token-and-transport-encryption).
...
see [docs/behavior.md](behavior/board-api.md#get-apisession-token)). On the frontend, `useBoardStatus.ts`
...
`api.BoardRoutePrefix` sub-router. See [security.md](security/auth.md#auth-token-and-transport-encryption).
