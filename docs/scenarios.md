...
| I9 | A URL with no callback port, or a local pane | No forward is opened, and the reason says why | `auto`: `internal/api/openurl_test.go`; the route's own wiring in `internal/server/openurl_routes_test.go` |
