...
| J11b | Issues and Jira keys of a task | The issues the task's PR closes, and the Jira keys in its branch name or PR title linked into `task_dashboard.jira_url`, are linked from the card and the detail panel in a new tab; none without a PR (issues) or without the setting (Jira); a stopped task shows neither; a non-`https` Jira site is refused at load | `auto`: `internal/api` — `TestTaskGitInfo_IssuesThePRClosesAndJiraKeys`, `TestTaskGitInfo_NoJiraSiteNoJiraLinks`, `TestTaskGitInfo_WithoutAPROnlyTheBranchGivesJiraKeys`, `TestTaskGitInfo_WithoutGHTheBranchStillGivesJiraKeys`, `TestJiraKeys`, `TestTaskGitFor`; `internal/config` — `TestValidate_TaskDashboardJiraURL`, `TestJiraBrowseURL`; `frontend/src/components/TaskDashboard.test.tsx`. `manual`: that `gh pr view --json closingIssuesReferences` lists a real PR's closing issues — open a task whose branch has a PR with `Closes #<n>` in its body and confirm the issue appears |
