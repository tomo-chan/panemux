...
| J11b | Issues and references of a task | The issues the task's PR closes, and the references `task_dashboard.autolinks` finds in its branch name or PR title (GitHub's autolink shape: `key_prefix`, `url_template` with `<num>`, `is_alphanumeric`), are linked from the card and the detail panel in a new tab; none without a PR (issues) or without the setting (references); only configured prefixes match; a stopped task shows neither; a non-`https` template, one with `<num>` in the host, one whose host or port the browser would not parse, or overlapping prefixes are refused at load; with a `gh` older than 2.72.0 the PR link stays and issues are absent | `auto`: `internal/api` — `TestTaskGitInfo_IssuesThePRClosesAndAutolinks`, `TestTaskGitInfo_NoAutolinksNoLinks`, `TestTaskGitInfo_WithoutAPROnlyTheBranchGivesAutolinks`, `TestTaskGitInfo_WithoutGHTheBranchStillGivesAutolinks`, `TestTaskGitInfo_AGHWithoutClosingIssuesStillFindsThePR`, `TestTaskGitInfo_NoPRRunsGHOnce`, `TestAutolinkRefs`, `TestTaskGitFor`; `internal/config` — `TestValidate_TaskDashboardAutolinkURLTemplate`, `TestValidate_TaskDashboardAutolinkKeyPrefix`, `TestAutolinkURL_ReplacesEveryPlaceholder`, `TestLoad_ReadsTaskDashboardAutolinks`; `frontend/src/components/TaskDashboard.test.tsx`; `frontend/src/schemas/index.test.ts`. `manual`: that `gh pr view --json closingIssuesReferences` lists a real PR's closing issues — open a task whose branch has a PR with `Closes #<n>` in its body and confirm the issue appears |
| J12 | Filtering and rows | Text filter over directory, branch, PR, reference and session; rows by host or repository | `auto`: `frontend/src/utils/taskBoard.test.ts`; `frontend/src/components/TaskDashboard.test.tsx` |
