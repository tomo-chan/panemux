...
| I6b | An `ssh` pane whose remote login shell is not POSIX (`fish`, `tcsh`) | The pane starts its login shell, with the shim installed and `PANEMUX_PANE_ID` exported, as it does under `bash` | `auto`: `internal/session` — `TestSSHShellCommandIsOneLineForSh`, `TestSSHShellCommandRunsUnderEveryLoginShell` (each shell that is not installed is skipped); a real sshd with `fish` and `tcsh` users was checked by hand (see the decision log) |
...
sessions on every host, by state, independently of panes. The e2e rows run the real collection
...
| J7 | Where a task runs | An agent under a tmux pane, however deep, is located in that tmux session; one outside tmux in no panemux pane says so and offers no pane | `auto`: `internal/tasks` — `TestBuildTasks_LocationFollowsTheParentChainToATmuxPane`; `frontend/e2e/task-dashboard.spec.ts` |
| J7b | Opening an agent outside tmux | An agent started from a `local` pane, or from an `ssh` pane on the task's host, offers `Go to pane` and switches to that pane; a pane ID naming a pane of another kind or host, or one no workspace holds, opens nothing; the ID comes from `PANEMUX_PANE_ID`, which those panes export and collection reads from the agent's environment on Linux hosts and checks | `auto`: `internal/session` — `TestPaneIDEnvAcceptsOnlySafePaneIDs`, `TestPaneIDEnvDropsAnInheritedPaneID`, `TestNewLocalExportsThePaneID`, `TestSSHShellCommandExportsThePaneIDWhenRunByAShell`, `TestSSHSessionLifecycleOverInProcessTransport`; `internal/tasks` — `TestParseCollectOutput_KeepsOnlyPaneIDsOfTheShapeAPaneIsGiven`, `TestBuildTasks_LocationCarriesThePaneIDOfAnAgentOutsideTmux`, `TestRunLocal_ReportsThePaneIDInAnAgentsEnvironment`; `frontend/src/utils/taskBoard.test.ts`; `frontend/src/components/TaskDashboard.test.tsx`; `frontend/e2e/task-dashboard.spec.ts` (Linux only) |
...
| J12 | Filtering and rows | Text filter over directory, branch, PR and session; rows by host or repository | `auto`: `frontend/src/utils/taskBoard.test.ts`; `frontend/src/components/TaskDashboard.test.tsx` |
| J12b | Another site's page requests the task routes | Refused with `403` before anything is collected or reconnected | `auto`: `internal/api` — `TestTaskRoutes_RefuseCrossSiteRequests` |
...
| J13 | The routes through the real router | `GET /api/tasks` and the reconnect route answer through `server.New()` | `auto`: `internal/server` — `TestServer_APIIntegration` |
