...
| H12b3 | A `#123` reference is printed in a pane whose repository is known | It links to that repository's pull request, at the row and columns it actually occupies — the same 1-based mapping the URL provider uses, so a reference inside a URL is resolved in the URL's favour rather than landing on a different row entirely | `auto`: `frontend/src/hooks/useTerminalLinks.test.ts` — `pull request link provider` |
...
| H21 | Adding an SSH host without leaving the dashboard | The button beside the pane settings connection picker opens panemux's own dialog; saving it writes the host to the user's ssh config and closes the dialog; a write that fails keeps the dialog open and says why, including when the failure carries no message; and the refreshed list offers the new alias to the pane it was added for, which then connects on the next settings save (B3) | `auto`: `frontend/src/App.test.tsx`, `frontend/src/components/AddSSHHostDialog.test.tsx`, `frontend/src/hooks/usePaneSettings.test.ts` |
