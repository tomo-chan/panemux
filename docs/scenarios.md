...
| E11 | The board MCP server answers a request | Every response carries exactly one of `result`/`error`, and a parse error answers with an explicit null id rather than none — JSON-RPC 2.0 §5, not merely what today's client tolerates | `auto`: `internal/boardmcp` — `TestInitializedSentAsARequestIsAnsweredWithANullResult`, `TestAnErrorResponseCarriesNoResultMember`, `TestServerMalformedLineReturnsParseError` |
