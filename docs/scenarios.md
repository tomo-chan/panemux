...
| J4 | A leftover state file | A file whose pid is gone, or now belongs to a process that is not claude, is not shown as running | `auto`: `internal/tasks` — `TestBuildTasks_StateFileNeedsALiveClaudeProcess` |
| J5 | An unreadable state file, and codex | Kept on the board as unknown rather than dropped; a running codex process shows as running | `auto`: `internal/tasks` — `TestBuildTasks_UnreadableStateFileIsKeptAsUnknown`, `TestBuildTasks_CodexProcessesAreRunningTasks` |
...
| J11 | Repository, branch and PR of a task | Looked up once per directory and cached, linked from the card and the detail panel | `auto`: `internal/api` — `TestTaskGitInfo_LocalRepositoryWithPR`, `TestGetTasks_GitLookupsAreSharedPerDirectoryAndCached`; `frontend/src/components/TaskDashboard.test.tsx` |
