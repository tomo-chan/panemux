...
| E9 | A query succeeds but its history cannot be written | The answer stays on screen and the turn reads as finished, with the write failure shown as a warning under it — not as a failed turn, and not silently | `auto`: `internal/commandcenter` — `TestQueryReportsAFailedHistoryWriteAsAWarningOnTheDoneFrame`, `TestQueryReportsNoWarningsWhenTheHistoryWriteSucceeds`, `TestQueryKeepsOneTerminalEventWhenBothTheTurnAndTheHistoryWriteFail`; on the wire in `internal/ws` — `TestDoneEventCarriesItsWarningsOntoTheFrameAndOmitsThemOtherwise`; in the UI, `frontend/src/components/CommandPalette.test.tsx` and `frontend/src/hooks/useBoardCommand.test.ts` |
