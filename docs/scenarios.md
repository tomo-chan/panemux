...
| J11b | Issues and Jira keys of a task | The issues the task's PR closes, and the Jira keys in its branch name or PR title linked into `task_dashboard.jira_url`, are linked from the card and the detail panel in a new tab; none without a PR (issues) or without the setting (Jira); a stopped task shows neither; a non-`https` Jira site, or one whose host or port the browser would not parse, is refused at load; with a `gh` older than 2.72.0 the PR link stays and issues are absent | `auto`: `internal/api` — `TestTaskGitInfo_IssuesThePRClosesAndJiraKeys`, `TestTaskGitInfo_NoJiraSiteNoJiraLinks`, `TestTaskGitInfo_WithoutAPROnlyTheBranchGivesJiraKeys`, `TestTaskGitInfo_WithoutGHTheBranchStillGivesJiraKeys`, `TestTaskGitInfo_AGHWithoutClosingIssuesStillFindsThePR`, `TestTaskGitInfo_NoPRRunsGHOnce`, `TestJiraKeys`, `TestTaskGitFor`; `internal/config` — `TestValidate_TaskDashboardJiraURL`, `TestJiraBrowseURL`; `frontend/src/components/TaskDashboard.test.tsx`; `frontend/src/schemas/index.test.ts`. `manual`: that `gh pr view --json closingIssuesReferences` lists a real PR's closing issues — open a task whose branch has a PR with `Closes #<n>` in its body and confirm the issue appears |
