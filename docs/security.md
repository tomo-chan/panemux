...
| Sections | Document |
|---|---|
| Shell path; Tmux session name; Remote path arguments; SSH private key paths and an unresolvable home directory; Launching the operator's browser (`--open`); Task dashboard collection | [command-execution.md](security/command-execution.md) |
